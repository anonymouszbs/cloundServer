...
<body>
    <div class="yadmin-body animated fadeIn">
        <form class="layui-form layui-form-pane" action="" lay-filter="thisForm">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="pwd" id="pwd" name="pwd" placeholder="看不到密码，但可修改密码" class="layui-input">
                </div>
            </div>
            ...
            <div class="layui-form-item" pane="">
                ...
                <div class="layui-input-block">
                    <input type="radio" name="AllowedLogin" value="1" title="正常">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <select id="role" lay-filter="role" name="role" xm-select="role-select" lay-verify="required" lay-vertype="tips">
            ...
            <div class="layui-form-item">
                <div class="layui-input-block">
                    <button class="layui-btn layui-btn-normal" lay-submit="" lay-filter="submit">更新</button>
                </div>
    ...
    <script>
        layui.use(['form', 'tree'], function() {
            ...
            $('#user_nick').val("{$selectdata['user_nick']}");
            $('#id_card_number').val("{$selectdata['id_card_number']}");
            $('#WorkPermitNum').val("{$selectdata['WorkPermitNum']}");
            $('#username').val("{$selectdata['username']}");

            $('input[name="AllowedLogin"][value="{$selectdata["AllowedLogin"]}"]').attr('checked', "checked");



            $('#role').val("{$selectdata['role']}");
            $('#Department').val('{$selectdata["Department"]}');
            form.render('select');
            form.render('radio');
            form.on('submit(submit)', function(data) {
                ...
                $.post("{$config['admin_route']}Api/up_user_info?id={$selectdata['id']}", data.field, function(res) {
                    layer.close(index);
                    var code = res.code;
                    var msg = res.msg;
                    switch (code) {
