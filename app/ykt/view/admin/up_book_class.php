...
<body>
    ...
    <script>
        layui.use(['form', 'tree'], function() {
            ...
            $ = layui.jquery;
            $('#NodeName').val("{$selectdata['NodeName']}");
            $('#ParentID').val("{$selectdata['ParentID']}");
            $('#sortid').val("{$selectdata['sortid']}");
            form.render('select')
            form.on('submit(submit)', function(data) {
                ...
                $.post("{$config['admin_route']}Api/up_content_tree?id={$selectdata['id']}", data.field, function(res) {
                    layer.close(index);
                    var code = res.code;
                    var msg = res.msg;
                    switch (code) {
